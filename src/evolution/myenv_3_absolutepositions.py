...
class MyGridWorld(ParallelEnv):
    ...
    def __init__(self, render_mode=None):
        ...
        OBSERVATION_DIM = 2*len(self.agents)+2*2+1
        ...
        for a, pos in self.agent_positions.items():
            if pos[1]<4:
                ...
                if self.render_mode == "human":
                    print("Agent on upper area")
            if pos[1]<1+3 or (pos[0]==self.button_pos[0] and pos[1]==self.button_pos[1]):
                rewards[a] = 0
                if self.render_mode == "human":
                    print("Agent on button")
